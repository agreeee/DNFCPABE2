...
void PRF_Const(unsigned char Pmsk[][32], int f_psi[][dt], bool f_phi[][dt],
	...
	for (i = 0; i < 32; i++) {
		sk_f[0][i] = sk_f[0][i] ^ Pmsk[cd[1] * DT][i];
		sk_f[1][i] = sk_f[1][i] ^ Pmsk[cd[1] * DT][i];
		sk_f[2][i] = sk_f[2][i] ^ Pmsk[cd[1] * DT + 1][i];
		sk_f[3][i] = sk_f[3][i] ^ Pmsk[cd[1] * DT + 1][i];
		for (j = 0; j < DT; j++) {
			sk_f[j][i] = sk_f[j][i] ^ Pmsk[cd[2] * DT + j][i];
		}
	}
	//S_2
	for (i = 0; i < WT; i++) {
	...
	for (i = 0; i < 32; i++) {
		sk_f[y[0]][i] = sk_f[y[0]][i] ^ Pmsk[cd[0] * DT + 1][i];
		sk_f[y[1]][i] = sk_f[y[1]][i] ^ Pmsk[cd[0] * DT + 1][i];
		sk_f[y[2]][i] = sk_f[y[2]][i] ^ Pmsk[cd[0] * DT + 3][i];
		sk_f[y[3]][i] = sk_f[y[3]][i] ^ Pmsk[cd[0] * DT + 3][i];

		sk_f[y[0]][i] = sk_f[y[0]][i] ^ Pmsk[cd[2] * DT + 2][i];
		sk_f[y[1]][i] = sk_f[y[1]][i] ^ Pmsk[cd[2] * DT + 3][i];
		sk_f[y[2]][i] = sk_f[y[2]][i] ^ Pmsk[cd[2] * DT + 2][i];
		sk_f[y[3]][i] = sk_f[y[3]][i] ^ Pmsk[cd[2] * DT + 3][i];
	}
	//S_3
	for (i = 0; i < WT; i++) {
		y[i] = y[i] + WT;
	...
	for (i = 0; i < 32; i++) {
		for (j = 0; j < DT; j++) {
			sk_f[y[j]][i] = sk_f[y[j]][i] ^ Pmsk[cd[0] * DT + j][i];
		}

		sk_f[y[0]][i] = sk_f[y[0]][i] ^ Pmsk[cd[1] * DT + 1][i];
		sk_f[y[1]][i] = sk_f[y[1]][i] ^ Pmsk[cd[1] * DT + 3][i];
		sk_f[y[2]][i] = sk_f[y[2]][i] ^ Pmsk[cd[1] * DT + 1][i];
		sk_f[y[3]][i] = sk_f[y[3]][i] ^ Pmsk[cd[1] * DT + 3][i];
	}
